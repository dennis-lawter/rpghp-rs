...
* WIP Switch from hyphenated to simple when viewing every UUID...
    - Consider even further reducing this hex view to base64 or a similarly highly compressed string
...
** TODO Add the current httpyac e2e test to git automation or github actions
...
* TODO Consider switching from double uuidv4 creation to a hashing algorithm
    ...
    - It could be that non-v4 UUIDs could create entropy though (for the secret), use one without time-based encoding
...
** DONE Introduce a trait for the ApiResponses to enforce from_domain_error
